...
export default function AgentPage() {
  ...
  const handleEdit = (arrangement: Arrangement) => {
    setFormData({
      ...
      startDate: arrangement.startDate.split('T')[0],
      endDate: arrangement.endDate.split('T')[0],
      numberOfNights: arrangement.numberOfNights.toString(),
      imageUrl: arrangement.imageUrl || '',
      categoryId: arrangement.categoryId.toString()
    })
  ...
  const handleDeleteDiscount = async (discountId: number) => {
    if (!confirm('Da li ste sigurni da želite da obrišete ovaj popust?')) {
      return
    }
    ...
    try {
      ...
      if (response.ok) {
        setMessage('Popust uspešno obrisan!')
        fetchArrangements()
      } else {
        const data = await response.json()
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage('Aranžman uspešno obrisan!')
        fetchArrangements()
      } else {
        const data = await response.json()
        setMessage(data.error || 'Greška pri brisanju')
      ...
      <div className="flex gap-4 mb-6">
        <button
          ...
          className={`px-4 py-2 rounded-lg ${
            activeTab === 'arrangements' ? 'bg-purple-600 text-white' : 'bg-gray-200'
          }`}
        >
          Aranžmani
        </button>
        ...
        <div>
          ...
          {showForm && (
            <div className="bg-white p-6 rounded-lg shadow-md mb-8">
              <h2 className="text-xl font-semibold mb-4">
                {editingId ? 'Izmeni aranžman' : 'Kreiraj novi aranžman'}
              </h2>
              <form onSubmit={handleSubmit}>
                ...
                <div className="flex gap-4 mt-6">
                  <Button type="submit">
                    {editingId ? 'Sačuvaj izmene' : 'Kreiraj aranžman'}
                  </Button>
            ...
            <div className="bg-white p-6 rounded-lg shadow-md mb-8">
              ...
              <form onSubmit={handleDiscountSubmit}>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  <Input
                    label={discountData.type === 'percentage' || discountData.type === 'lastMinute' ? 'Vrednost (%)' : 'Vrednost (€)'}
                    name="value"
                    type="number"
                ...
                <div className="flex gap-4 mt-6">
                  ...
                  <Button type="button" variant="secondary" onClick={resetDiscountForm}>
                    Otkaži
                  </Button>
                </div>
              </form>
          ...
          <div className="bg-white rounded-lg shadow-md overflow-hidden">
                  ...
                  <tr key={arr.id} className="border-b hover:bg-gray-50">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex gap-2">
                        ...
                        <button
                          onClick={() => handleAddDiscount(arr.id)}
            ...
            {arrangements.length === 0 && (
              <p className="text-center text-gray-500 py-8">Nema aranžmana.</p>
            )}
          </div>
        </div>
      )}
